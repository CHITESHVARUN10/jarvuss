...
final class VoiceAuthClient {
    ...
    // MARK: - OCR + RAG helpers

    /// Sends POST /ocr/scan for a file already copied into ./rag_documents/.
    /// Returns the raw JSON dict (keys: "text", "pages", etc.).
    func ocrScan(filename: String) async throws -> [String: Any] {
        var request = URLRequest(url: baseURL.appendingPathComponent("ocr/scan"))
        request.httpMethod = "POST"
        request.setValue("application/json", forHTTPHeaderField: "Content-Type")
        let body = try JSONSerialization.data(withJSONObject: ["filename": filename])
        request.httpBody = body
        ...
        do {
            ...
            return json
        } catch let err as VoiceAuthClientError {
            throw err
        } catch {
            throw VoiceAuthClientError.backendUnavailable
        }
    ...
    /// Sends POST /rag/ingest with an optional specific filename filter.
    /// Pass nil to ingest all new documents in ./rag_documents/.
    func ragIngest(filename: String?) async throws -> [String: Any] {
        var request = URLRequest(url: baseURL.appendingPathComponent("rag/ingest"))
        request.httpMethod = "POST"
        request.setValue("application/json", forHTTPHeaderField: "Content-Type")
        var bodyDict: [String: Any] = [:]
        if let fn = filename { bodyDict["filename"] = fn }
    ...
    /// Sends POST /rag/query to retrieve top-k matching text chunks for a query string.
    func ragQuery(query: String, topK: Int = 3) async throws -> [String] {
        var request = URLRequest(url: baseURL.appendingPathComponent("rag/query"))
        request.httpMethod = "POST"
        request.setValue("application/json", forHTTPHeaderField: "Content-Type")
        let bodyDict: [String: Any] = ["query": query, "top_k": topK]
        ...
        do {
            ...
            return results.compactMap { $0["document"] as? String }
        } catch let err as VoiceAuthClientError {
            throw err
        } catch {
            throw VoiceAuthClientError.backendUnavailable
